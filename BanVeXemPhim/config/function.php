...
{
    if ($page == 'admin') {
        define('BASE_URL', 'http://localhost:3000/BanVeXemPhim/admin/');
    } else {
        define('BASE_URL', 'http://localhost:3000/BanVeXemPhim/');
    }
...
function getAllByCondition($table, $column, $value) {
    global $conn;

    // Chuẩn bị truy vấn SQL
    $query = "SELECT * FROM $table WHERE $column = ? AND TrangThai = 1";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("i", $value); // Giả sử $value là số nguyên (integer)
    $stmt->execute();
    $result = $stmt->get_result();
