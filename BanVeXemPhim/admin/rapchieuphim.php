...
$pagination = setupPagination($conn, 'rapchieuphim', $records_per_page, $searchString, 'r.*, k.TenKhuVuc', 'rapchieuphim r LEFT JOIN khuvuc k ON r.MaKhuVuc = k.MaKhuVuc');
    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center pb-0">
                <h5>Danh sách rạp chiếu phim</h5>
                ...
                <a href="views/rapchieuphim/rapchieuphim-add.php" class="btn btn-lg me-5 btn-add"
                    ...
                    <table class="table table-striped table-borderless align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center text-uppercase text-xs font-weight-bolder">Tên rạp</th>
                                <th class="text-center text-uppercase text-xs font-weight-bolder">Địa chỉ</th>
                                <!-- <th class="text-center text-uppercase text-xs font-weight-bolder">Khu vực</th> -->
                        ...
                        <tbody>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="text-center text-xs font-weight-bolder"><?= $row['TenRap'] ?></td>
                                        <td class="text-center text-xs font-weight-bolder"><?= $row['DiaChi'] ?></td>
                                        <!-- <td class="text-center text-xs font-weight-bolder"><?= $row['TenKhuVuc'] ?></td> -->
                                        <td class="text-center text-s font-weight-bolder">
                                            <form action="controllers/rapchieuphim-controller.php" method="POST" style="display:inline;">
                                                <input type="hidden" name="marap" value="<?= $row['MaRap'] ?>">
                                                <input type="hidden" name="status" value="<?= $row['TrangThai'] == 1 ? 0 : 1 ?>">
                                                <button type="submit" name="changeStatus"
                                                    class="badge badge-sm <?= $row['TrangThai'] == 1 ? 'bg-gradient-success' : 'bg-gradient-secondary' ?> text-uppercase"
                                                    style="border: none; cursor: pointer;">
                                                    <?= $row['TrangThai'] == 1 ? 'ON' : 'OFF' ?>
                                                </button>
                                            </form>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            <a class="btn btn-info m-0"
                                                ...
                                                href="views/rapchieuphim/rapchieuphim-edit.php?id=<?= $row['MaRap'] ?>">
                                                <i class="bi bi-pencil"></i> Sửa
                                            </a>
                                            <a class="btn btn-danger m-0 delete-btn" data-id="<?= $row['MaRap'] ?>"
                                                data-url="controllers/rapchieuphim-controller.php?action=delete"
                                                style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;"
