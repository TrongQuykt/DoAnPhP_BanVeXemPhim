...
<div class="row">
    ...
    value="<?php echo isset($formData['dao_dien']) ? htmlspecialchars($formData['dao_dien']) : htmlspecialchars($film['data']['DaoDien']); ?>"
    placeholder="Nhập tên đạo diễn">
                    </div>
                    <div class="form-group mb-3">
                        <label for="dien_vien">Diễn viên</label>
                        <input type="text" class="form-control" id="dien_vien" name="dien_vien"
    value="<?php echo isset($formData['dien_vien']) ? htmlspecialchars($formData['dien_vien']) : htmlspecialchars($film['data']['DienVien']); ?>"
    placeholder="Nhập tên diễn viên">
                ...
                <div class="col-md-6">
                    ...
                    <!-- Thêm trường nhập URL trailer -->
                    <div class="form-group mb-3">
                        <label for="trailer">Link Trailer (YouTube)</label>
                        <input type="text" class="form-control" id="trailer" name="trailer"
                            value="<?php echo isset($formData['trailer']) ? htmlspecialchars($formData['trailer']) : htmlspecialchars($film['data']['Trailer'] ?? ''); ?>"
                            placeholder="Nhập URL trailer (ví dụ: https://www.youtube.com/watch?v=...)">
                        <?php if (isset($messages['trailer'])): ?>
                            <small class="text-danger m-2 text-xs"><?= htmlspecialchars($messages['trailer']) ?></small>
                        <?php endif; ?>
                    </div>
...
document.getElementById('editFilmForm').addEventListener('submit', function(event) {
    var trailerInput = document.getElementById('trailer').value.trim();
    var youtubeRegex = /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/.+$/;

    if (trailerInput !== '' && !youtubeRegex.test(trailerInput)) {
        event.preventDefault();
        alert('URL trailer không hợp lệ! Vui lòng nhập link YouTube hợp lệ (ví dụ: https://www.youtube.com/watch?v=...)');
