...
if (isset($_GET['code'])) {
    ...
    $client_secret = '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';
    ...
    if (isset($token_data['error'])) {
        redirect('views/login.php', 'error', 'Đăng nhập thất bại: ' . htmlspecialchars($token_data['error']));
    }
    ...
    if (isset($token_data['access_token'])) {
        ...
        if ($user_info === false) {
            redirect('views/login.php', 'error', 'Đăng nhập thất bại: Không thể lấy thông tin người dùng.');
        }
              ...
              VALUES ('$tendn', '', '$name', '0')";
                ...
                if (mysqli_query($conn, $query)) {
                    ...
                    if (mysqli_query($conn, $insert_query)) {
                        ...
                        redirect('views/login.php', 'error', 'Đăng nhập thất bại: Không thể thêm vào NguoiDung.');
                    }
                } else {
                    redirect('views/login.php', 'error', 'Đăng nhập thất bại: Không thể thêm vào TaiKhoan.');
                }
            }
        } else {
            redirect('views/login.php', 'error', 'Đăng nhập thất bại: Không lấy được thông tin người dùng.');
        }
    } else {
        redirect('views/login.php', 'error', 'Đăng nhập thất bại: Không có access token.');
    }
} else {
    redirect('views/login.php', 'error', 'Đăng nhập thất bại: Không có mã phản hồi.');
}
